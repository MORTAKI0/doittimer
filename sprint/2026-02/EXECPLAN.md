...
| 2026-02-14 | Focus consistency implementation: added realtime `/focus` subscriptions (`sessions`, `tasks`, `task_queue_items`), cross-tab broadcast accelerator, leader-gated interval bell, global widget sync, route-refresh scheduler + event deduper, and migration `20260214_s2026_02_focus_realtime_publication.sql`. |
...
| 2026-02-08 | Added migration `supabase/migrations/20260208_s2026_02_tasks_by_date_fields.sql` to support tasks-by-date filtering: nullable `tasks.scheduled_for` (date), nullable `tasks.completed_at` (timestamptz), and composite indexes `(user_id, scheduled_for)` + `(user_id, completed_at)`. No app code changes. |
| 2026-02-08 | Centralized task date-completion backend path: added `setTaskScheduledFor(taskId, scheduledFor)` with strict `YYYY-MM-DD|null` validation and owner-scoped update; added `setTaskCompleted(taskId, completed)` updating `completed` + `completed_at`; refactored `TaskList` completion toggle to call `setTaskCompleted` directly; kept compatibility wrapper `toggleTaskCompletion`. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-02-08 | Added scheduling UI in task rows (native date input + Today + Clear), wired to `setTaskScheduledFor` with optimistic updates, rollback on error, and inline error display. Added scheduled date badge near title. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-02-08 | US3 bugfix: replaced `getTasks` RPC dependency (`get_tasks_page`) with direct owner-scoped table query + count/range pagination to ensure fresh `scheduled_for/completed_at` data and stable list sizing; removed `router.refresh()` in per-row schedule updates to avoid optimistic overwrite flicker; clamped `/tasks` `limit` search param to minimum 5 to prevent accidental one-item views. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-02-08 | US4: added URL-driven Tasks filters (status, scheduled quick range/day picker, project) and backend filtered pagination/count. Implemented same-filter exact count + paged items query in `getTasks`, week/day scheduled filters with include-unscheduled default true, and page clamp redirect when requested page exceeds filtered total pages. Moved project/archive filtering off client `TaskList` to server-side filter path; added empty filtered state with clear-filters action. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-02-08 | Auto-archive completed tasks: added `user_settings.auto_archive_completed` migration + settings action and Settings UI toggle; updated `setTaskCompleted` to load flag, set `completed_at`, auto-set `archived_at` when enabled, owner-scope remove queue item (`task_queue_items` by `user_id` + `task_id`), and revalidate `/tasks` + `/focus` + `/settings`. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `pnpm dev` boot check blocked by existing `.next/dev/lock` (another dev server already running). |
| 2026-02-08 | Dashboard V1 (hybrid): replaced dashboard with URL-backed range selector and KPI/Today plan/Queue panels; added `getDashboardSummary({ range, from?, to? })` with timezone-aware [from,to) boundaries using `user_settings.timezone` fallback `UTC`, KPI aggregation (created/completed/archived/queue/completionRate/onTimeRate), today scheduled+unscheduled slices, and queue preview; added `/dashboard` revalidation to task and queue mutations; added additive index migration `20260208_s2026_02_dashboard_v1_indexes.sql`. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `pnpm dev -- --hostname 127.0.0.1 --port 3011` blocked by existing `.next/dev/lock` (another dev server already running). |
| 2026-02-08 | Cross-app UI/UX polish pass: added global typography tokens + optional SF Pro gate via generated `app/font-sans.generated.ts` (fallback to system stack), standardized UI primitives (`Card*`, `Button` variants, `Select`, `Tooltip`, `EmptyState`, `KpiCard`, `Skeleton`), refreshed Dashboard/Tasks/Focus/Settings/Auth/Public page hierarchy and empty/loading states, and cleaned UTF-8 text corruption in public/settings surfaces. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `pnpm build` OK. |
| 2026-03-16 | Frontend foundation refactor pass: updated `app/globals.css` to the new Todoist-like token set (light/dark), reduced radius scale to 4/6/8px, added semantic nav/task/divider/focus utility classes, removed deprecated animation usage (`fadeInUp`, `slideInRight`, `floatSoft`, non-focus `glowPulse`), flattened sidebar/task surfaces, and switched shared UI primitives plus app shell/task list to the new rules. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `pnpm dev` boot check timed out in Codex before a manual smoke confirmation. |
| 2026-03-16 | Frontend page-shell/navigation pass: added task navigation summary helpers and wired the desktop/mobile app shell to the spec order with Inbox/Today counts, Focus queue count, and dynamic project links. Added `/inbox`, `/today`, `/upcoming`, `/filters-labels`, and `/completed` task-shell routes, kept Dashboard/Focus full width, and used explicit backend-dependent messaging only for labels data that is not modeled yet. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-03-16 | Frontend task experience pass: rewrote `TaskList` around flat Todoist-style rows, added inline quick-add with date/priority/project controls, introduced frontend task presentation persistence for `priority`/`description` until backend schema exists, migrated Tasks filters/projects surfaces away from card-heavy treatment, and updated Focus queue rows to the new task-row anatomy. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-03-16 | Dev reliability fix: made service worker registration production-only, auto-unregisters stale DoItTimer service workers/caches in development, tightened `sw.js` to bypass `/_next` and force network-first navigations, added `no-store` headers to `/api/sessions/active`, and added dev-only polling backoff/pause in `GlobalRunningSessionWidget` to stop repeated fetch noise when localhost is unavailable. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-03-16 | Added dev-only runtime diagnostics for nav clicks, pathname/search transitions, traced fetches (`_rsc` and `/api/sessions/active`), service worker state, and server-side app/auth/session timing logs. Findings in this environment: sandboxed `pnpm dev` failed with `next dev` -> `spawn EPERM`, which left `localhost:3000` unavailable; escalated `pnpm dev` served requests normally, and `/api/sessions/active` responded quickly with `Auth session missing!` rather than hanging. Verification: `pnpm lint` OK; `pnpm typecheck` OK. |
| 2026-03-18 | Automation tokens foundation + settings management: added additive Supabase migrations for `automation_tokens` and `create_automation_token`, server-only automation auth/response helpers, `/api/agent/health` + `/api/agent/me`, Settings token create/revoke UI, env/example updates for service-role auth, and new unit/E2E coverage files. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `node --test tests/unit/automation-tokens.test.ts` blocked by the repo's raw alias-unaware runner; `pnpm dev` boot check timed out in Codex before manual smoke confirmation. |
| 2026-03-28 | US-03 V2 labels: added labels/task_labels migration with owner-only RLS and normalized uniqueness, shared label services/actions, task query hydration + label filtering, Filters & Labels CRUD UI, searchable task-edit assignment, TaskRow label pills/overflow, and label unit/E2E coverage files. Verification: `pnpm lint` OK; `pnpm typecheck` OK; `node --test tests/unit/labels.test.ts` blocked by the repo's raw TS module runner; manual `pnpm dev` smoke not run in this session. |
