...
- Sessions RPC names are used by server actions (do not rename):
...
- Code calls `get_dashboard_today_stats` via `app/actions/dashboard.ts` but repo has no migration file for it.

**Non-breaking rule:** do not rename RPC functions. Change internals only. Keep return shapes compatible.
...
## Why we pick this
"Today" boundaries depend on timezone, and defaults reduce friction (start focus faster). This also fixes the current hard-coded timezone bug.
...
- `upsert_user_settings(p_timezone text, p_default_task_id uuid)`
  ...
  - validate timezone non-empty (DB-level simplest):
...
Add a migration for `get_dashboard_today_stats`:
...
- Today sessions + dashboard stats use the user timezone (not hardcoded)
